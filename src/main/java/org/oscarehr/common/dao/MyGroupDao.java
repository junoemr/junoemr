...
public class MyGroupDao extends AbstractDao<MyGroup> {
	...
	{
		Query query = entityManager.createQuery("SELECT g FROM MyGroup g, ProviderData p WHERE p.id = g.id.providerNo AND  p.status = 1 ORDER BY g.id.myGroupNo,length(g.id.mygroup_no), g.id.providerNo");
		return query.getResultList();
	}
