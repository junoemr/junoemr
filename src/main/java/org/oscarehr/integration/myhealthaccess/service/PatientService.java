...
public class PatientService extends BaseService
{
	...
	/**
	 * get MHA patient by "account Id code". These codes are temporary codes (lasting at the time of writing 5 minutes)
	 * generated by the patient that allow the clinic to connection & look at there account record even when access
	 * would normally be blocked. This means the patient record returned by this search may be completely unconnected to this clinic
	 * as such other methods you may call on this patient will fail with InvalidAccessException.
	 * @param integration - the integration to lookup the patient in
	 * @param idCode - the patient's temporary id code
	 * @return the patient if found or null
	 */
	public MHAPatient getPatientByAccountIdCodeCode(Integration integration, String idCode)
	{
		try
		{
			RestClientBase restClient = RestClientFactory.getRestClient(integration);
			...
			queryParams.add("search_by", "account_id_code");
			queryParams.add("code", idCode);
			...
			{
				throw new RecordNotFoundException("Could not find MHA patient with account id code: " + idCode + " Did it expire?");
			}
			else if (response.isNotUnique())
			{
				throw new RecordNotUniqueException("Multiple patients with account id code: " + idCode + " This is likely a bug in the application logic. Check MHA.");
			}
		}
		catch (InvalidIntegrationException e)
		{
