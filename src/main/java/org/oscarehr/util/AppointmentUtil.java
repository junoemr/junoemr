...
public class AppointmentUtil {
	...
	public static String getLastAppointment(String demographicNo){
		Date nextApptDate = null;
	       if (demographicNo != null && !demographicNo.equalsIgnoreCase("") && !demographicNo.equalsIgnoreCase("null")){
	           try {
	              String sql = "select max(appointment_date) as appointment_date from appointment where demographic_no = '"+demographicNo+"' and status not like '%C%' and CONCAT(IFNULL(appointment_date, ''), ' ', IFNULL(start_time, '')) < now() order by appointment_date";
	              ResultSet rs = DBHandler.GetSQL(sql);
	              if (rs.next()) {
	                 nextApptDate = rs.getDate("appointment_date");
	              }
