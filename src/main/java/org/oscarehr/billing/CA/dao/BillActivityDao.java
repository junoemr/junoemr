...
public class BillActivityDao extends AbstractDao<BillActivity>{
	...
	public List<BillActivity> findCurrentByMonthCodeAndGroupNo(String monthCode, String groupNo, Date updateDateTime) {
		Query q = entityManager.createQuery("SELECT b FROM BillActivity b WHERE b.monthCode=?1 AND b.groupNo=?2 AND b.updateDateTime > ?3 AND b.status != 'D' ORDER BY b.batchCount");
	...
	public List<BillActivity> findCurrentByDateRange(Date startDate, Date endDate) {
		Query q = entityManager.createQuery("SELECT b FROM BillActivity b WHERE b.updateDateTime >= ?1 AND  b.updateDateTime <= ?2 AND b.status != 'D' ORDER BY b.id DESC");
