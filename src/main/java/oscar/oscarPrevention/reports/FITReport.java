...
public class FITReport extends PreventionsReport{
	...
	public Hashtable<String,Object> runReport(LoggedInInfo loggedInInfo, ArrayList<ArrayList<String>> list, Date asofDate){
		...
		for (int i = 0; i < list.size(); i ++){//for each  element in arraylist
			...
			ArrayList<Map<String,Object>> prevs = PreventionData.getPreventionData(loggedInInfo, "FIT",demo);
			PreventionData.addRemotePreventions(loggedInInfo, prevs, demo,"FIT",null);
		...
		h.put("eformSearch","FIT");
		h.put("followUpType","FITF");
