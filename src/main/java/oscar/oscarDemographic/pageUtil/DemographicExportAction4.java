...
import org.oscarehr.healthReportManager.dao.HRMDocumentCommentDao;
import org.oscarehr.healthReportManager.dao.HRMDocumentDao;
import org.oscarehr.healthReportManager.dao.HRMDocumentToDemographicDao;
...
import org.oscarehr.healthReportManager.model.HRMDocumentToDemographic;
			...
			case CMS4 :
				if (!Util.checkDir(tmpDir)) {
					...
					for (String demoNo : list) {
						...
						String rosterDate = "";
						if (demographic.getRosterDate() != null) rosterDate = formatter.format(demographic.getRosterDate());
						String rosterTermDate = "";
						if (demographic.getRosterTerminationDate() != null) rosterTermDate = formatter.format(demographic.getRosterTerminationDate());
						...
						if (StringUtils.filled(rosterStatus)) {
							...
							if (rosterStatus.equals("1")) {
								...
								if (UtilDateUtilities.StringToDate(rosterTermDate) != null) enrolment.setEnrollmentTerminationDate(Util.calDate(rosterTermDate));
								String termReason = demographic.getRosterTerminationReason();
								if (StringUtils.filled(termReason)) enrolment.setTerminationReason(cdsDt.TerminationReasonCode.Enum.forString(termReason));
							}
						}
						...
						for (int i = 0; i < DAs.size(); i++) {
							...
							if (i == 0) { // check history info with current
								...
								if (historyRS.equals(rosterStatus) && rd.equals(rosterDate) && td.equals(rosterTermDate)) continue;
							}
							else { // check history info with next
								if (historyRS.equals(historyRS1) && UtilDateUtilities.nullSafeCompare(historyRD, historyRD1) == 0
							...
							if (historyRS.equals("1")) {
								...
								if (historyTD != null) enrolment.setEnrollmentTerminationDate(Util.calDate(historyTD));
						...
						if (demographic.getPatientStatusDate() != null) patientStatusDate = formatter.format(demographic.getPatientStatusDate());
						...
						if (StringUtils.filled(demographic.getHin())) {
							...
							else healthCard.setProvinceCode(cdsDt.HealthCardProvinceCode.X_70); // Asked,
																								// unknown
							...
							String HCRenewDate = "";
							if (demographic.getHcRenewDate() != null) HCRenewDate = formatter.format(demographic.getHcRenewDate());
						...
						for (CaseManagementNote cmn : lcmn) {
							if (cmn.getNote() != null && cmn.getNote().startsWith("imported.cms4.2011.06") && cmm.getLinkByNote(cmn.getId()).isEmpty()) headers.add(cmn);
						}
						...
						if (exMedicationsAndTreatments) {
							...
							for (int p = 0; p < arr.length; p++) {
								...
								/*
								 * no need: DrugReasonDao drugReasonDao =
								 * (DrugReasonDao)
								 * SpringUtils.getBean("drugReasonDao");
								 * List<DrugReason> drugReasons =
								 * drugReasonDao.getReasonsForDrugID(arr[p].
								 * getDrugId(), true); if (drugReasons.size()>0
								 * &&
								 * StringUtils.filled(drugReasons.get(0).getCode
								 * ())) medi.setProblemCode(drugReasons.get(0).
								 * getCode());
								 *
								 */
								...
								if (dosageValue == 0) { // takemin=0, try
														// takemax
								...
								/*
								 * no need: if (arr[p].getNosubs())
								 * medi.setSubstitutionNotAllowed("Y"); else
								 * medi.setSubstitutionNotAllowed("N"); mSummary
								 * = Util.addSummary(mSummary,
								 * "Substitution not Allowed",
								 * arr[p].getNosubs()?"Yes":"No");
								 *
								 */
								...
								/*
								 * no need: if
								 * (StringUtils.filled(arr[p].getRxStatus())) {
								 * medi.setPrescriptionStatus(arr[p].getRxStatus
								 * ()); mSummary = Util.addSummary(mSummary,
								 * "Prescription Status", arr[p].getRxStatus());
								 * } /* no need: if
								 * (arr[p].getDispenseInterval()!=null) {
								 * medi.setDispenseInterval(String.valueOf(arr[p
								 * ].getDispenseInterval())); mSummary =
								 * Util.addLine(mSummary, "Dispense Interval",
								 * arr[p].getDispenseInterval().toString()); }
								 *
								 */
								...
								if (StringUtils.filled(outsideProviderName)) {
									...
									if (StringUtils.filled(prescribeProvider)) {
										...
										mSummary = Util
												.addSummary(mSummary, "Prescribed by", StringUtils.noNull(prvd.getFirst_name()) + " " + StringUtils.noNull(prvd.getLast_name()));
									}
								}

								/*
								 * no need: data = arr[p].isNonAuthoritative() ?
								 * "Y" : "N";
								 ...
								 * medi.getPrescriptionIdentifier());
								 *
								 */
						...
						if (exLaboratoryResults) {
							 ...
							 * List<LabMeasurements> labMeaList =
							 * ImportExportMeasurements.getLabMeasurements(
							 * demoNo); for (LabMeasurements labMea :
							 ...
							 * hl7TextMessage =
							 * hl7TxtMssgDao.find(Integer.valueOf(lab_no));
							 * String hl7Body = new
							 * String(Base64.decodeBase64(hl7TextMessage.
							 * getBase64EncodedeMessage())); MessageHandler h =
							 * Factory.getHandler(hl7TextMessage.getType(),
							 * hl7Body); for (int i=0; i<h.getOBRCount(); i++) {
							 ...
							 * labMeaValues.put("name", h.getOBXName(i, j));
							 * labMeaValues.put("labname",
							 * h.getPatientLocation());
							 * labMeaValues.put("datetime", h.getTimeStamp(i,
							 * j)); labMeaValues.put("abnormal",
							 * h.getOBXAbnormalFlag(i, j));
							 * labMeaValues.put("measureData", commentAsResult);
							 * labMeaValues.put("unit", h.getOBXUnits(i, j));
							 * labMeaValues.put("accession",
							 * h.getAccessionNum()); labMeaValues.put("range",
							 * h.getOBXReferenceRange(i, j));
							 * labMeaValues.put("request_datetime",
							 * h.getRequestDate(i));
							 * labMeaValues.put("olis_status",
						...
						if (exReportsReceived) {
							...
							for (int j = 0; j < edoc_list.size(); j++) {
								...
								if (!f.exists()) {
									...
									while ((numRead = in.read(b, offset, b.length - offset)) >= 0 && offset < b.length)
										offset += numRead;
