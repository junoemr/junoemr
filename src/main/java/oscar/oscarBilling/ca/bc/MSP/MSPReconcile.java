...
//	}
	...
	public ResultSet getMSPRemittanceQuery(String payeeNo, String s21Id) {
		...
		String qry = "SELECT billing_code,provider.first_name,provider.last_name,t_practitionerno,t_s00type,billingmaster.service_date as 't_servicedate',t_payment," + "t_datacenter,billing.demographic_name,billing.demographic_no,teleplanS00.t_paidamt, GROUP_CONCAT(distinct t_exp1),GROUP_CONCAT(distinct t_exp2),GROUP_CONCAT(distinct t_exp3),GROUP_CONCAT(distinct t_exp4),GROUP_CONCAT(distinct t_exp5),GROUP_CONCAT(distinct t_exp6),t_dataseq " + " from teleplanS00,billing,billingmaster,provider " + " where teleplanS00.t_officeno = billingmaster.billingmaster_no " + " and teleplanS00.s21_id = " + s21Id
