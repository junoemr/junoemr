...
public class AddEFormAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String prev_fdid = (String)session.getAttribute("eform_data_id");
		...
		{
			Integer oldFdid = Integer.parseInt(prev_fdid);
			eForm = eFormService.saveExistingEForm(oldFdid, demographicNo, providerNo, formId, subject, formOpenerMap, paramValueMap, eform_link);
		}
		else
		{
			eForm = eFormService.saveNewEForm(demographicNo, providerNo, formId, subject, formOpenerMap, paramValueMap, eform_link);
		}

		boolean sameForm = (eForm == null);
		String fdid = (sameForm)? prev_fdid : eForm.getId().toString();


		//post fdid to {eform_link} attribute
		if(!sameForm && eform_link != null)
		{
			session.setAttribute(eform_link, fdid);
		}
		if(fax)
		{
...
//			paramValueMap.put(curField, request.getParameter(curField));
//		}
//
//	}
//
...
//		String val = (String)session.getAttribute(lnk);
//		openerValues.add(val);
//		if (val!=null) session.removeAttribute(lnk);
//	}
//
//	//----names parsed
...
//		eFormService.saveEformValues(new Integer(fid), new Integer(fdid), new Integer(demographic_no), paramValueMap);
//
//		//post fdid to {eform_link} attribute
//		if (eform_link!=null) {
//			session.setAttribute(eform_link, fdid);
...
//		else {
//			//write template message to echart
//			String program_no = new EctProgram(session).getProgram(providerNo);
//			String path = request.getRequestURL().toString();
//			String uri = request.getRequestURI();
//			path = path.substring(0, path.indexOf(uri));
//			path += request.getContextPath();
