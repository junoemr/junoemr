 ...
 * 
 * @deprecated use {@link org.oscarehr.provider.model.ProviderData} instead
 */
public class ProviderData {
	...
	*/
	 ...
	 */
	private org.oscarehr.provider.model.ProviderData toProvider() {
		org.oscarehr.provider.model.ProviderData result = new org.oscarehr.provider.model.ProviderData();
	...
	private void fromProvider(org.oscarehr.provider.model.ProviderData p) {
		provider_no = p.getId();
	...
	public List<String> getProviderListWithInsuranceNo(String insurerNo) {
		...
		List<org.oscarehr.provider.model.ProviderData> providers = dao.findByTypeAndOhip(PROVIDER_TYPE_DOCTOR, insurerNo);
	...
	public void getProvider(String providerNo) {
		ProviderDataDao dao = SpringUtils.getBean(ProviderDataDao.class);
		org.oscarehr.provider.model.ProviderData provider = dao.findByProviderNo(providerNo);
		if (provider != null) {
	...
	**/
	...
	public static List<Map<String, String>> searchProvider(String searchStr, boolean onlyActive) {
		...
		ProviderDataDao dao = SpringUtils.getBean(ProviderDataDao.class);
		List<org.oscarehr.provider.model.ProviderData> providers = dao.findByName(firstname, lastname, onlyActive);
	...
	public static List<Map<String, String>> getProviderListOfAllTypes(boolean inactive) {
		...
		List<org.oscarehr.provider.model.ProviderData> providers = dao.findAll(inactive);
		for(org.oscarehr.provider.model.ProviderData p : providers) {
			result.add(toMap(p));
		}
		return result;
	...
	public static String getProviderName(String providerNo) {
		ProviderDataDao dao = SpringUtils.getBean(ProviderDataDao.class);
		org.oscarehr.provider.model.ProviderData p = dao.findByProviderNo(providerNo);
		if (p == null)
			return "";
	...
	public void getProviderWithOHIP(String ohipNo) {
		if (filled(ohipNo)) {
			ProviderDataDao dao = SpringUtils.getBean(ProviderDataDao.class);
			org.oscarehr.provider.model.ProviderData p = dao.findByOhipNumber(ohipNo);
			if (p != null)
				fromProvider(p);			
		}
	}
	...
	public void getProviderWithNames(String firstName, String lastName) {
		if (filled(firstName) && filled(lastName)) {
			ProviderDataDao dao = SpringUtils.getBean(ProviderDataDao.class);
			List<org.oscarehr.provider.model.ProviderData> providers = dao.findByName(firstName, lastName, false);
