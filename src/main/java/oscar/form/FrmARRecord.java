...
public class FrmARRecord extends FrmRecord {
	public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID) throws SQLException {
		...
		if (existingID <= 0)
		{
			String sql = "SELECT demographic_no, CONCAT(last_name, ', ', first_name) AS pName, sex, CONCAT(address, ', ', city, ', ', province, ' ', postal) AS address, phone, phone2, year_of_birth, month_of_birth, date_of_birth " +
					"FROM demographic WHERE demographic_no = " + demographicNo;
			ResultSet rs = DBHandler.GetSQL(sql);
			if (rs.next())
			{
				String yyyy = oscar.Misc.getString(rs, "year_of_birth");
				String mm = oscar.Misc.getString(rs, "month_of_birth");
				String dd = oscar.Misc.getString(rs, "date_of_birth");
				java.util.Date date = UtilDateUtilities.calcDate(yyyy,mm,dd);
				...
				props.setProperty("pg1_dateOfBirth", ConversionUtils.toDateString(date, "yyyy/MM/dd"));
				props.setProperty("pg1_age", String.valueOf(UtilDateUtilities.calcAge(yyyy,mm,dd)));
				...
				props.setProperty("pg1_formDate", ConversionUtils.toDateString(new Date(), "yyyy/MM/dd"));
			}
			rs.close();
		}
		else
