...
public class FrmCaregiverRecord extends FrmRecord {
	...
	public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID)
		...
		if (existingID <= 0)
		{
			sql = "SELECT demographic_no FROM demographic WHERE demographic_no=" + demographicNo;
		...
		{
			sql =   "SELECT * FROM formCaregiver WHERE demographic_no=" + demographicNo + " AND ID=" + existingID;
			rs = DBHandler.GetSQL(sql);
			...
			if (rs.next())
			{
				...
				for (int i = 1; i <= md.getColumnCount(); i++)
				{
					...
					else
					{
						...
						{
							value = ConversionUtils.toDateString(rs.getDate(i),"yyyy/MM/dd");
						}
						else
						{
	...
	public int saveFormRecord(Properties props) throws SQLException {
		...
		String sql =
			"SELECT * FROM formCaregiver WHERE demographic_no="
				+ demographic_no
				+ " AND ID=0";
