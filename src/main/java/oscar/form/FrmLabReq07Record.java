	...
	public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID) throws SQLException {
		...
		if (existingID <= 0)
		{
			...
			if (demographic != null)
			{
				...
				props.setProperty("birthDate", StringUtils.trimToEmpty(ConversionUtils.toDateString(dob, "yyyy/MM/dd")));
			...
			try
			{
				...
				if (directLinks.size() > 0)
				{
					...
					if (p != null)
					{
						...
						{
							if (providerPk.getIntegratorFacilityId() == cp.getFacilityIdIntegerCompositePk().getIntegratorFacilityId()
									&& "OSCAR".equals(cp.getName()) &&  cp.getAddress() != null)
							{
