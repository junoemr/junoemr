...
public class FrmSelfAdministeredRecord extends FrmRecord {
	...
	public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID)
		...
		if (existingID <= 0)
		{
			sql = "SELECT demographic_no, sex, year_of_birth, month_of_birth, date_of_birth FROM demographic WHERE demographic_no = "
                              + demographicNo;
			...
			if (rs.next())
			{
				...
				props.setProperty("dob", UtilDateUtilities.DateToString(dob,_dateFormat));
		...
		{
			sql =
				"SELECT * FROM formSelfAdministered WHERE demographic_no = "
					+ demographicNo
					+ " AND ID = "
					+ existingID;
			rs = DBHandler.GetSQL(sql);
