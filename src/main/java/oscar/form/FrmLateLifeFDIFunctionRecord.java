...
public class FrmLateLifeFDIFunctionRecord extends FrmRecord {
	...
	public Properties getFormRecord(LoggedInInfo loggedInInfo, int demographicNo, int existingID)
		...
		{
			sql = "SELECT * FROM formLateLifeFDIFunction WHERE demographic_no=" + demographicNo + " AND ID=" + existingID;
			rs = DBHandler.GetSQL(sql);
	...
	public int saveFormRecord(Properties props) throws SQLException {
		...
		String sql =
			"SELECT * FROM formLateLifeFDIFunction WHERE demographic_no="
				+ demographic_no
				+ " AND ID=0";
