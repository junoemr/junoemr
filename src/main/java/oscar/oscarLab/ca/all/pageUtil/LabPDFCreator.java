...
public class LabPDFCreator extends PdfPageEventHelper{
    ...
    public void printPdf() throws IOException, DocumentException{
        ...
        ByteArrayOutputStream temp_os = new ByteArrayOutputStream();
        PdfWriter writer = PdfWriter.getInstance(document, temp_os);
    ...
    public static byte[] stamp(byte[] content) throws Exception {
		...
		for (int i = 1; i <= reader.getNumberOfPages(); i++) {
			...
			printText(over, "Personal Health Information: CONFIDENTIAL", center, 25);
		}
				...
				for (int k = 0; k < obxCount; k++) 
				{
					...
					if (!handler.getOBXResultStatus(j, k).equals("TDIS")) 
					{
						...
						if ((!handler.getOBXResultStatus(j, k).equals("DNS")
								...
								|| (handler.getMsgType().equals("PFHT") && !obxName.equals("") && handler.getObservationHeader(j,k).equals(header))) 
						{ // <<-- DNS only needed for
													// MDS messages
							...
							else
							{
								...
								if(!isLongText)
								{//if the Abn, RR and Unit columns have not been occupied above
									if(handler.getMsgType().equals("PATHL7")){
										cell.setPhrase(new Phrase(
    ...
    public void onEndPage(PdfWriter writer, Document document){
        try {
            ...
            int numPages = document.getPageNumber();
...
/* Generate this after the pdf is created using a stamper so we can print the total number of pages
            //add footer for every page
            cb.beginText();
            cb.setFontAndSize(bf, 8);
            cb.showTextAligned(PdfContentByte.ALIGN_CENTER, "Page "+pageNum+" of " + numPages, width/2, 30, 0);
            cb.endText();
