...
<%@ page import="org.oscarehr.report.reportByTemplate.model.ReportTemplates" %>
...
<%
	...
	ReportTemplatesDao templateDao = SpringUtils.getBean(ReportTemplatesDao.class);
%>
	...
	<table class="MainTable" id="scrollNumber1" name="encounterTable">
		...
		<tr>
			...
			<%
				...
				Integer templateId;
				String templatexml = "";
				ReportTemplates template = null;

				String templateIdStr = StringUtils.trimToEmpty((String) request.getAttribute("templateid"));
				if(templateIdStr.isEmpty())
				{
					templateIdStr = StringUtils.trimToEmpty(request.getParameter("templateid"));
				}

				MiscUtils.getLogger().info("templateIdStr=" + templateIdStr);
				if(!templateIdStr.isEmpty())
				{
					templateId = Integer.parseInt(templateIdStr);
					template = templateDao.find(templateId);
					templatexml = template.getTemplateXml();
					action = "edit";
				}
			...
			<td class="MainTableRightColumn" valign="top">
				...
				<input type="hidden" name="templateid" value="<%=templateIdStr%>">
				<input type="submit"
				...
				<div id="editXml"
					...
					<br/>
					<%
						boolean isVerified = (template != null) && template.isAdminVerified();
						if(provider.isSuperAdmin())
						{ %>
					<div>
						<input type="checkbox" name="admin_verified" value="true" <%=isVerified? "checked":""%>>
						<label>Verified Query</label>
					</div>
					<%
						}
					%>

					<input type="button" onclick="document.forms[1].submit()" value="<%
					if (action.equals("edit"))
					{
						out.print("Save");
					}
					else{
						out.print(StringUtils.capitalize(action));
					}%>">
					...
					{
					%>
					<input type="button" name="done" value="Done" onclick="document.location='reportConfiguration.jsp?templateid=<%=templateIdStr%>'">
					<%
					}
					else
					{
