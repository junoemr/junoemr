	...
	<table>
		...
		<tr>
			...
			<td>
				...
				<% if (demographicLink != null) { %>
					<oscar:nameage demographicNo="<%=String.valueOf(demographicLink.getDemographicNo()) %>" /> <a href="#" onclick="removeDemoFromHrm('<%=hrmReportId %>')">(remove)</a>
				<% } else { %>
		...
		<tr>
			...
			<td>
				<%
				List<HrmObservation> hrmObservationList = hrmReport.getObservations();
				List<HRMDocumentSubClass> subClassListFromDb = (List<HRMDocumentSubClass>) request.getAttribute("subClassList");

				if (hrmObservationList.size() > 0) {
				%>
				<i>From the Report</i><br />
					<% for (HrmObservation subClass : hrmObservationList) { %>
						<abbr title="Type: <%= subClass.getAccompanyingSubClass() %>
						; Date of Observation: <%=(ConversionUtils.toDateString(subClass.getObservationDateTime())) %>">
							(<%= subClass.getAccompanyingMnemonic() %>) <%= subClass.getAccompanyingDescription() %>
						</abbr><br />
					<% }
				} %><br />
				<%
