...
//TODO-legacy: determine dependencies
...
dojo.xml.Parse = function(){
	...
	function getDojoTagName(node){
		...
		if(tagName.substr(0,4) == "dojo"){
			// FIXME-legacy: this assumes tag names are always lower case
		...
		// <tag dojo:type="type"> => dojo:type
		try{
			// FIXME-legacy: IE really really doesn't like this, so we squelch errors for it
		...
		if((dj_global["djConfig"])&&(!djConfig["ignoreClassNames"])){ 
			// FIXME-legacy: should we make this optionally enabled via djConfig?
			var classes = node.className||node.getAttribute("class");
			// FIXME-legacy: following line, without check for existence of classes.indexOf
	...
	this.parseElement = function(	/*DomNode*/node,
		...
		if(process){
			...
			// FIXME-legacy: we might want to make this optional or provide cloning instead of
	...
	this.parseAttributes = function(/*DomNode*/node){
		...
		// TODO-legacy: should we allow for duplicate attributes at this point...
