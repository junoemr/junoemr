	...
	<table class="layouttable">
		...
		<tr>
			...
			<td><select name="docClass" id="docClass">
				...
				%>
				<option value="<%=reportClass%>" <%=(formdata.getDocClass() != null && reportClass.equals(formdata.getDocClass())) ? "selected" : ""%>><%=reportClass%>
				</option>
				<% } %>
			</select>
			</td>
		...
		<tr>
			...
			<td>
				<select name="responsibleId">
					...
					%>
					<option value="<%=pd.get("providerNo")%>" <%=selected%>><%=pd.get("lastName")%>, <%=pd.get("firstName")%>
					</option>
					<% } %>
				</select>
			</td>
		...
		<tr>
			<td>File Name:</td>
			<td>
				<div style="width: 300px; overflow: hidden; text-overflow: ellipsis;"><%=fileName%>
				</div>
		</tr>
		<tr>
