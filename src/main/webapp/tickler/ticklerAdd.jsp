...
<html:html locale="true">
		...
		<tr bgcolor="#000000">
			...
			<td width="90%" align="left">
				<p><font face="Verdana, Arial, Helvetica, sans-serif" color="#FFFFFF"><b><font
						...
						key="tickler.ticklerAdd.msgTickler"/></font></b></font>
				</p>
			</td>
		</tr>
	</table>
	<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#EEEEFF">
		<form name="ADDAPPT" method="post" action="../appointment/appointmentcontrol.jsp">
			...
			<%
				// This is linked to a form label which specifies a name search, so we will give the user one here
				// rather than using the default search mode specified in the parameters file.
			...
			<INPUT TYPE="hidden" NAME="appointment_date" VALUE="2002-10-01" WIDTH="25" HEIGHT="20"
				   border="0" hspace="2">
			<INPUT TYPE="hidden" NAME="status" VALUE="t" WIDTH="25" HEIGHT="20" border="0"
				   hspace="2">
			...
			<INPUT TYPE="hidden" NAME="duration" VALUE="15" WIDTH="25" HEIGHT="20" border="0"
				   hspace="2">
			<INPUT TYPE="hidden" NAME="end_time" VALUE="10:59" WIDTH="25" HEIGHT="20" border="0"
				   hspace="2" onChange="checkTimeTypeIn(this)">


			<input type="hidden" name="demographic_no" readonly value="" width="25" height="20"
				   border="0" hspace="2">
			<input type="hidden" name="location" tabindex="4" value="" width="25" height="20"
				   border="0" hspace="2">
			<input type="hidden" name="resources" tabindex="5" value="" width="25" height="20"
				   border="0" hspace="2">
			<INPUT TYPE="hidden" NAME="user_id" readonly VALUE='oscardoc, doctor' WIDTH="25"
			...
			<tr>
				...
				<td valign="top">
					<select name="priority" style="font-face:Verdana, Arial, Helvetica, sans-serif">
						<option value="<bean:message key="tickler.ticklerMain.priority.high"/>"><bean:message
								key="tickler.ticklerMain.priority.high"/>
						<option value="<bean:message key="tickler.ticklerMain.priority.normal"/>"
								SELECTED><bean:message key="tickler.ticklerMain.priority.normal"/>
						<option value="<bean:message key="tickler.ticklerMain.priority.low"/>"><bean:message
...
%>
					<select name="task_assigned_to">
						...
						%>
						<option value="<%=proOHIP%>" <%=selected%> ><%=proLast%>, <%=proFirst%>
						</option>
						<%
							}
						%>
					...
					<input type="hidden" name="docType"
						   value="<%=request.getParameter("docType")%>"/>
