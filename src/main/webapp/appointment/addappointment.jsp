...
<%
	...
	boolean authed = true;
%>
<security:oscarSec roleName="<%=roleName$%>" objectName="_appointment" rights="w"
				   reverse="<%=true%>">
	<%authed = false; %>
...
<html:html locale="true">
	<head>
		...
		<script>
			function checkPageLock()
			{ //don't do anything unless timeout/locking is enabled.
			}
	...
	<table width="98%" <%=rsbgcolor%> border=0 align='center'>
		<tr>
			<td><font color='blue' title='<%=exp%>'> <b><bean:message
					key="Appointment.msgPatientStatus"/>:&nbsp;
				<font color='yellow'><%=patientStatus%>
				</font>&nbsp;<bean:message key="Appointment.msgRosterStatus"/>:&nbsp;
				<font color='yellow'><%=rosterStatus%>
				</font></b></font>
			</td>
		</tr>
	</table>
	...
	<table width="98%" BGCOLOR="yellow" border=1 align='center'>
		<tr>
			<td><font color='red'><bean:message key="Appointment.formAlert"/>:
				<b><%=demographicCust.getAlert()%>
				</b></font></td>
		</tr>
	</table>
	<%
		...
		<tr>
			...
			%> <bean:message key='appointment.addappointment.msgBooking'/>
				...
				%> <bean:message
						...
						if (bDnb)
							out.println("<br/>You can NOT book an appointment on this time slot.");
					}
	...
	<table width="98%" align="center"
		   style="border:solid 3px red;padding-left:10px;line-height:150%;font-family:Arial;color: red; background-color: #FFFFFF; font-size: 18px; font-weight: bold; text-align:left;">
	...
	<FORM NAME="ADDAPPT" id="addappt" METHOD="post"
		...
		<div class="header deep">
			<div class="title">
				...
				<% if (isMobileOptimized)
				{ %><bean:message key="appointment.addappointment.msgMainLabelMobile"/>
				<% } else
				{ %><bean:message key="appointment.addappointment.msgMainLabel"/>
		...
		<div class="panel">
			<ul>
				<li class="row weak">
					<div class="label"><bean:message key="Appointment.formDate"/><font size='-1'
																					   color='brown'>(<%=dateString1%>
						)</font>:
					</div>
					<div class="input">
				...
				<li class="row weak">
					...
					%>
					<div class="input" style="text-align: right;"><INPUT TYPE="button"
																		 NAME="typeButton"
				...
				<li class="row deep">
					<div class="label"><bean:message
							key="appointment.addappointment.formSurName"/>:
					</div>
					<div class="input">
						<%
							String name = "";
					...
					<div class="space">
						<a href=# onclick="onNotBook();"><font size='-1' color='brown'>Not
							book</font></a>
					</div>
					...
					<INPUT TYPE="hidden" NAME="originalpage"
						   VALUE="../appointment/addappointment.jsp">
					...
					<div class="input">
						...
						<select id="site-select" tabindex="4" name="location" style="background-color: <%=colo%>"
							...
							<option value="<%=s.getName()%>"
									style="background-color: <%=s.getBgColor()%>" <%=s.getName().equals(loc) ? "selected" : "" %>><%=s.getName()%>
							</option>
							<% } %>
						</select>
						<% } else
						...
						<select name="location">
							...
							%>
							<option value="<%=program.getId()%>" <%=program.getId().toString().equals(sessionLocation) ? "selected='selected'" : ""%>><%=StringEscapeUtils.escapeHtml(description)%>
							</option>
							<% }
							}
							%>
						...
						<% } else
						{ %>
						<input type="TEXT" name="location" tabindex="4" value="<%=loc%>" width="25"
							   height="20" border="0" hspace="2">
						<% } %>
						<% } %>
					</div>
				...
				<li class="row weak">
					...
					<div class="input">
						...
						%>
						<INPUT TYPE="TEXT" NAME="createdatetime" readonly VALUE="<%=strDateTime%>"
							   WIDTH="25" HEIGHT="20" border="0" hspace="2">
				...
				<li class="row weak">
					...
					<div class="input"><input type="checkbox" name="emailPt" value="email reminder">
					</div>
					<% } else
					{ %>
					<div class="label"></div>
		...
		<table class="buttonBar deep">
			<tr>
				...
				<TD align="right">
					...
					<% }%>
					<INPUT TYPE="RESET" id="backButton" class="leftButton top"
						   VALUE="<bean:message key="appointment.addappointment.btnCancel"/>"
						   onClick="cancelPageLock();window.close();">
					<% if (!props.getProperty("allowMultipleSameDayGroupAppt", "").equalsIgnoreCase("no"))
					{%>
					<input type="button" id="apptRepeatButton"
		...
		<tr>
			<td valign="top">
				<%
					...
					{
				%>
				<table style="font-size: 9pt;" bgcolor="#c0c0c0" align="center" valign="top"
					   cellpadding="3px">
					<tr bgcolor="#ccccff">
						<th colspan="2">
							<bean:message key="appointment.addappointment.msgDemgraphics"/>
							<a title="Master File"
							   onclick="popup(700,1000,'../demographic/demographiccontrol.jsp?demographic_no=<%=demoNo%>&amp;displaymode=edit&amp;dboperation=search_detail','master')"
							   href="javascript: function myFunction() {return false; }"><bean:message
									key="appointment.addappointment.btnEdit"/></a>

							<bean:message key="appointment.addappointment.msgSex"/>: <%=sex%> &nbsp;
							<bean:message key="appointment.addappointment.msgDOB"/>: <%=dob%>
						</th>
					</tr>
					<tr bgcolor="#ccccff">
						<th style="padding-right: 20px" align="left"><bean:message
								key="appointment.addappointment.msgHin"/>:
						</th>
						<td><%=hin.replace("null", "")%>
						</td>
					</tr>
					<tr bgcolor="#ccccff">
						<th style="padding-right: 20px" align="left"><bean:message
								key="appointment.addappointment.msgAddress"/>:
						</th>
						<td><%=StringUtils.trimToEmpty(address)%>
							, <%=StringUtils.trimToEmpty(city)%>
							, <%=StringUtils.trimToEmpty(province)%>
							, <%=StringUtils.trimToEmpty(postal)%>
						</td>
					</tr>
					<tr bgcolor="#ccccff">
						<th style="padding-right: 20px" align="left"><bean:message
								key="appointment.addappointment.msgPhone"/>:
						</th>
						<td><b><bean:message
								key="appointment.addappointment.msgH"/></b>:<%=StringUtils.trimToEmpty(phone)%>
							<b><bean:message
									key="appointment.addappointment.msgW"/></b>:<%=StringUtils.trimToEmpty(phone2)%>
						</td>
					</tr>
					<tr bgcolor="#ccccff" align="left">
						<th style="padding-right: 20px"><bean:message
								key="appointment.addappointment.msgEmail"/>:
						</th>
			...
			<td valign="top">
				...
				<table style="font-size: 9pt;" bgcolor="#c0c0c0" align="center" valign="top"
					...
					<tr bgcolor="#c0c0c0" align="left">
						...
						{ %>
						<td><bean:message
								key="appointment.addappointment.msgFormNotCompleted"/></td>
						<% } %>
					</tr>
					<%
								}
			...
			<td valign="top">
				<table style="font-size: 8pt;" bgcolor="#c0c0c0" align="center" valign="top">
					...
					%>
					<tr bgcolor="#eeeeff">
						<td style="background-color: #CCFFCC; padding-right: 25px"><%=ConversionUtils.toDateString(a.getAppointmentDate())%>
						</td>
						<td style="background-color: #CCFFCC; padding-right: 25px"><%=ConversionUtils.toTimeString(a.getStartTime())%>
						</td>
						<td style="background-color: #CCFFCC; padding-right: 25px"><%=p.getFormattedName()%>
						</td>
						<td style="background-color: #CCFFCC;"><%=a.getStatus() == null ? "" : (a.getStatus().equals("N") ? "No Show" : (a.getStatus().equals("C") ? "Cancelled" : ""))%>
						</td>
					</tr>
					<%
						}
